...
**中止（Esc / Ctrl-C）は exit 130** で、何も変更しない。`q` での中止は用意していない
（一覧はタイプで絞り込めるので、文字キーに中止を割り当てられない）。
...
⚠️ **端末の高さがバンドル数に足りないと一覧はスクロールし、スクロールバーは出ない。**
先頭の項目が見えていないだけのことがあるので、数が合わなければ ↑ で戻って確かめる
（`llmtpl bundles` が全件を出すので、そちらと突き合わせるのが確実）。
