...
引数なしの `llmtpl` は、cwd の全フラグをチェックボックスで出し、`llmtpl.conf` を書き換えてから
apply する。
...
**一覧は通常画面で出す**（普通の CLI と同じ）。手前に印字したバンドルルートの行が
そのまま一覧の上に見え、終わった後もスクロールバックに残る。
⚠️ バンドル数が端末の行数を超えれば huh はスクロールし、**スクロールバーは出ない**。
数が合わなければ ↑ で戻る（全件は `llmtpl bundles` で確かめられる）。

**apply は cwd の 1 ターゲットだけ**に効く。`llmtpl apply` は配下まで降りるので、いま触っていない
兄弟ターゲットまで再生成してしまう。対話モードは conf を 1 つ書き換えるだけなので、apply も
1 つに絞り、他に何件あるかを最後に伝える。

**`llmtpl.conf` が無いディレクトリ**では、その旨を出してから作成の可否を尋ねる。先に配下の
ターゲット数を数え、1 件でもあれば警告する —— 設定リポジトリのルートのような場所に conf を作ると、
そのルート自体がターゲットになるため。最終確認まで何も書かないので、途中で中止してもファイルは
残らない。「はい」と答えた場合は、全フラグが既定のままでもファイルは作る（`llmtpl.conf` の存在が
ターゲットの目印そのものだから）。
