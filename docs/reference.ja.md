...
**一覧は代替画面（alternate screen）で出す。** 通常画面へ描くと、手前に出した行と
シェルのプロンプトが端末の高さを食う。代替画面ならフォームが画面を丸ごと使える。
⚠️ その代わり**手前に出した行は隠れる**ので、バンドルルートの場所は一覧の上（フォームの中）
にも出す。フォームを抜ければ通常画面の行がまた見える。
