...
cat proj/.claude/CLAUDE.md          # → expected-on.md ＋ 1 行目の生成マーカ

echo 'styleguide = false' > proj/llmtpl.conf
llmtpl apply --tpl-home ./llm-tpl proj
cat proj/.claude/CLAUDE.md          # → expected-off.md ＋ 生成マーカ
```
...
```
llmtpl: テンプレートの評価に失敗（未定義フラグ参照の可能性）: template: CLAUDE.md.tmpl:4:11: executing "CLAUDE.md.tmpl" at <{{template "styleguide/tone" .}}>: template "styleguide/tone" not defined
```
