...
**ディレクトリ 1 つ**に畳み、リポジトリ側は 1 行で ON / OFF する。この README の例はすべて
Claude Code だが、仕組みは特定ツールのレイアウトに依存しない —— `AGENTS.md` や `.cursor/`・
`.github/` も同じ規則で配れる。
...
![一連の流れを録画したもの。ツリーを見せ、llmtpl apply で機能が入り、この 1 行で漏れなく出ていく。](assets/demo.gif)
...
冒頭の図がこの取引で、4 ファイルの実体を 1 つに寄せる代わりに、リポジトリごとの判断が
1 行ずつに縮む。

バンドルに置いたものが同じ相対位置へ重なるだけ —— これがどのエージェントのレイアウトでも
同じ規則で配れる理由（→「何がどこへ配られるか」）。作者は Claude Code + 複数リポジトリ ×
14 バンドルで日常運用している。CLI のメッセージは日英対応で、ロケールから自動で決まる
（→ [reference](docs/reference.ja.md) の「CLI メッセージの言語」）。
...
3. **1 つの JSON の別々のキーへ、別々の出どころから書き込む** —— バンドルの `hooks` と
   プロジェクト自身の `model`（`settings.json`）。複数の機能が書き込む場合も同じ
...
| コマンド | 用途 |
|---|---|
| `llmtpl` | 対話モード。ターゲットとフラグを選んで apply する（下記）。端末以外（パイプ・CI）から呼ぶと help |
...
| `llmtpl bundles` | バンドル一覧（説明つき。説明はバンドル内の `bundle.conf` の description・任意。旧名 `flags` も可） |
...
バンドル置き場が親方向に無いときは、`llmtpl.conf` の予約キー `bundle_root` でパスを明示する。
探索の範囲・解決順の全体・`bundle_root` の細則は
[reference](docs/reference.ja.md#探索とバンドルルートの解決)。
...
- **clone 直後は `llmtpl apply` を 1 回叩く必要がある。** symlink は gitignore するので、hook の
  登録だけが届いてスクリプトの実体は来ない —— この状態の Claude Code は**黙って失敗する**
  （答えは返り exit 0、畳まれた通知が 1 行出るだけ）。`llm-tpl/` をリポジトリ内に置けば 1 手で済む

退避の仕組み・`~/.claude` を生成物にするときの代償・保証しないことの全リスト・制約は
[reference](docs/reference.ja.md#使う上での細則) にある。clone 後に何が起きるかを測った結果は、
同じ reference の「チームで使うと、静かに壊れる場所がある」。
