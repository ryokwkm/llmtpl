...
複数あってよく、2 件以上なら件数を表示する。暴走は 2 段で止める —
`llm-tpl/` `partials/` `.archive/` `node_modules` `vendor` `testdata` `examples` は見ない /
**ドット始まりはターゲットにせず中へも降りない**。別リポジトリ（`.git` を持つディレクトリ）の
**中へも降りる** —— `llmtpl.conf` の存在そのものが opt-in なので、conf を置いていない
リポジトリが巻き込まれることはない。複数リポジトリの親ディレクトリから叩けば、各リポジトリの
ターゲットをまとめて扱える。

バンドルルートの解決順は `--tpl-home` → `$LLMTPL_HOME` → **`llmtpl.conf` の `bundle_root`** →
親を辿って `llm-tpl/` → `${XDG_CONFIG_HOME:-~/.config}/llmtpl`。
**解決はターゲットごと**（conf は自分のもの、親探索の起点は自分のディレクトリ）で、
`apply` / `check` / `status` はルートごとに「バンドルルート:」の行を出す。設定リポジトリの中で
...
- **効くのはそれを書いた conf のターゲットだけ**。ターゲットごとに別の値を書いてよく、
  それぞれ自分の置き場から合成される（同じ実行に混ざっても競合しない）
...
`partials/` には**名前付きブロック**も置ける。1 つのバンドルから複数の断片を、ターゲットが選んだ
場所へ差し込むための仕組みで、動く例が [examples/named-blocks](../examples/named-blocks/) にある
（テスト済み）。唯一にして最大の落とし穴も併記してある —— **`{{template}}` は必ず `{{if}}` で包む**。
包まないと、そのフラグを OFF にした瞬間に `apply` がそのターゲットと以降のターゲットごと失敗する。
...
引数なしの `llmtpl` は **2 段方式**。cwd 配下のターゲットが 1 つならそのままフラグ選択へ、
複数なら「ターゲットを選ぶ（↑↓ + enter）→ そのフラグを選ぶ → 確認 → `llmtpl.conf` を書き換えて
そのターゲットへ apply → 一覧へ戻る」を終わるまで繰り返す。一覧の各行には
いま ON のフラグの要約が付き、フラグ選択の見出しにはそのターゲットのバンドルルートが付く
（ルートが違うターゲットはそれぞれ自分の棚のフラグが出る）。

**esc の意味は段で違う**: ターゲット一覧では「終わる」（編集済みの分は適用済みなので exit 0）、
フラグ選択・確認では「そのターゲットの中止」（一覧から来たなら一覧へ戻る。ターゲットが
1 つだけなら従来どおり exit 130）。

**1 画面に全ターゲットの区画を並べる形は試して、やめた**（`0a4e87d` → 本方式で置き換え）。
huh は ↑↓ を区画の端でクランプするので矢印で区画を跨げず、enter が「次の区画へ / 確定」の
二重の意味を持って案内が嘘になる。

**起動するのは stdin と stdout の両方が端末のときだけ。** パイプ・リダイレクト・CI では
従来どおり help を出して exit 0 する —— 応答の来ないフォームはスクリプトを止めてしまう。

**中止（Esc / Ctrl-C）は exit 130** で、何も変更しない。huh の既定は Ctrl-C だけなので
Esc を足してある。そのために一覧の絞り込みは切ってある（huh が Esc を絞り込みの
設定・解除に使うため）。

**各行は 1 行に収まるよう切り詰める。** `bundle.conf` の説明は長いことが多く、折り返すと
一覧が想定する行数を超えて**先頭のバンドルが画面の外へ押し出される**。削るのは説明だけで、
フラグ名と既定 ON の注記は必ず残る。全文を読みたいときは端末を広げるか `llmtpl bundles` を使う。

**フォームは通常画面で出す**（普通の CLI と同じ）。終わった後は apply の出力
（バンドルルートの行 + ターゲットごとの結果）がスクロールバックに残る。
⚠️ バンドル数が端末の行数を超えれば huh はスクロールし、**スクロールバーは出ない**。
数が合わなければ ↑ で戻る（全件は `llmtpl bundles` で確かめられる）。

**配下に `llmtpl.conf` が 1 つも無いとき**だけ、その旨を出してから cwd への作成の可否を尋ねる
（1 件でもあればそれを出すだけで、親の階層に conf を増やす導線は作らない）。最終確認まで
何も書かないので、途中で中止してもファイルは残らない。「はい」と答えた場合は、全フラグが
既定のままでもファイルは作る（`llmtpl.conf` の存在がターゲットの目印そのものだから）。

conf は再生成せず、次の 4 つの規則で in-place に書き換える。

| そのフラグが | どうなるか |
|---|---|
| 行があり、既に望みの値 | 触らない |
| 行があり、値が違う | **値の文字だけ**を差し替える（桁揃え・インデント・行末の空白が残る） |
| 行が無く、既定値で望みに届く | 何も書かない |
| 行が無く、既定値では届かない | `name = true\|false` を末尾へ追記する |

**行は決して削除しない。** 明示 `false` と行が無い状態はパーサにとって同じ意味だが、消すと
その上のコメントが宙に浮く。同じ理由で、重複したキーは**最後の行だけ**を書き換え（パーサが
後勝ちなので、意味を決めているのはその行）、`bundle_root` はフラグとして扱わない。

書ける場面でも書かないのは意図的で、`llmtpl.conf` の 1 行は、後から `defaults.conf` を変えても
届かない場所が 1 つ増えることを意味するため。

### CLI メッセージの言語

CLI のメッセージはすべて日英の 2 言語を持ち、ロケールから自動で選ばれる。見るのは
`LLMTPL_LANG` > `LC_ALL` > `LC_MESSAGES` > `LANG` の順で最初の非空の値。`ja` で始まれば日本語、
それ以外（未設定を含む）は英語になる。llmtpl だけ言語を変えたいときは `LLMTPL_LANG` を使う
（`LLMTPL_LANG=en llmtpl apply`）。生成物は対象外で、`GENERATED` ヘッダは英語で固定
（マシンごとに揺れると git で衝突するため）。
