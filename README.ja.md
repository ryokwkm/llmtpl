...
| コマンド | 用途 |
|---|---|
| `llmtpl` | 配下の全ターゲットのフラグをチェックボックスで選んで apply する。端末以外（パイプ・CI）から呼ぶと help |
...
引数なしで叩くと、配下の全ターゲットをターゲットごとの区画（タイトル + バンドルルート +
チェックボックス）で出す。確定するとそれぞれの `llmtpl.conf` を書き換え、表示した全ターゲットへ
apply する。conf は**触る必要のある行の値だけ**を書き換えるので、フラグに添えたコメントは残る。
