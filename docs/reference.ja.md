...
引数なしの `llmtpl` は、cwd 配下の**全ターゲット**をターゲットごとの区画（タイトル +
バンドルルート + チェックボックス）で 1 つのフォームに出し、それぞれの `llmtpl.conf` を
書き換えてから、**表示した全ターゲットへ apply する**（`llmtpl apply` と同義。
画面に出したもの＝apply されるもの）。ルートが違うターゲットはそれぞれ自分の棚のフラグが出る。
...
**一覧は通常画面で出す**（普通の CLI と同じ）。各区画の見出しにバンドルルートが付き、
終わった後は apply の出力（ルートごとの行 + ターゲットごとの結果）がスクロールバックに残る。
⚠️ 区画の合計が端末の行数を超えれば huh はスクロールし、**スクロールバーは出ない**。
